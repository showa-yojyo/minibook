...
|Chapter06| では、段組、枠、表組、節の使用、文書内のページの向きの変更、境界と背
景の使用について説明がある。ページスタイルのその他の使い方については |Chapter08|
に説明がある。|PageStyleDlg| については |Chapter09| に説明がある。

.. note::

   Writer 文書のページはすべてページスタイルに基づいている。他のレイアウト方法
   (|Chapter06|) は、基本的なページスタイルに基づいて構築される。
...
|Chapter06| で説明されている技法（節、枠、表組）のいずれかを用いる。
...
|Chapter04| を参照しろ。また、|Chapter11| で説明されている技法のいずれかを使用し
て、ヘッダーとフッターに画像を追加したり、|Chapter06| で説明されているように、表
組を使って情報を並べたりすることもできる。

.. tip::

   Writer にはヘッダーとフッター用の段落スタイルが用意されており、他の段落スタイ
   ルと同じように変更して使用することができる。|Chapter08| と |Chapter09| を見
   ろ。
...
フィールドについては |Chapter17| で扱う。ここではある例が役に立つ。文書の表題を
