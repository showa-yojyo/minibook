...
.. |UpdateIndexC| replace:: :menuselection:`Update &Index`

.. tip::

   目次に見出しが表示されない場合は、正しい段落スタイルでタグ付けされているか確
   認しろ。見出しのレベル全体が表示されない場合は |HeadingNumberingM| の設定を確
   認しろ。|Writer08| を見ろ。

   テキストの追加や削除（見出しが別のページに移動するように）、見出しの追加、削
   除、変更を行った場合は、目次を更新する必要がある。これを行うには目次のどこか
   を右クリックし、コンテキストメニューから |UpdateIndexC| を選択する。
...
.. |InsertIndexEntryM| replace:: :menuselection:`&Insert-->Table of Contents and Inde&x-->&Index Entry...`
...
.. |EditIndexC| replace:: :menuselection:`Edit Inde&x`

目次は文書のスタイルや要件に合わせてカスタマイズすることができる。

まず、既存の目次の任意の場所で次のいずれかを行い、|TOCDlg| を開く：

* 右クリックメニューから |EditIndexC| を選択する
* 目次を表示する文書内でクリックし、|InsertTOCM| を選択
...
:guilabel:`&Headings`
   既定では、Writer は |HeadingNumberingM| で見出しレベルに関連付けられた段落ス
   タイルで書式整形された段落を使用する。
...
:guilabel:`Inde&x entries`
   この選択項目は |InsertIndexEntryM| を使用して文書にはめ込まれた索引登録項
...
* :guilabel:`N#` は |HeadingNumberingM| で見出しスタイルに割り当てられた見出し番
  号を表す。|Writer08| を参照しろ。
...
.. note::
   ...
   |HeadingNumberingDlg| (see |Writer08|) で、任意のレベルの
...
カスタム背景色の定義については |Writer20| を参照しろ。
...
から |EditIndexC| を選択する。また、|Sidebar| の Navigator を開き、
...
.. |DeleteIndexC| replace:: :menuselection:`Delete i&ndex`

文書から目次を削除するには、目次の任意の場所を右クリックし、コンテキストメニュー
から |DeleteIndexC| を選択する。また、Navigator から削除するには、右クリック
...
#. |InsertIndexEntryM| を選ぶか、または :guilabel:`Insert` ツールバーの
...
|InsertIndexEntryM| で :guilabel:`&Entry` に `Writer` と入力し、
...
#. 索引を追加したい文書内でクリックし、|InsertTOCM| を選択して |TOCDlg| を開く。
...
ニューから |EditIndexC| を選択する。|TOCDlg| が開き、前の節で記載されている五
つのタブを使用して索引を編集・保存することができる。

索引を更新または削除するには、索引内の任意の場所で右クリックし、|UpdateIndexC|
または |DeleteIndexC| をそれぞれ選択する。
...
#. 文書本文の既存の索引項目のフィールド網掛けにキャレットを置き、右クリックし
   て、内容メニューから |EditIndexC| を選択する。変更されたテキスト登録項目の
...
|InsertCaptionM| を使用するか、|Writer17| で説明するように数値範囲変数を使用して
手動で図版の説明を作成したならば、図版や図表の目次を作成するのは簡単だ。
...
.. |InsertBiblioEntryM| replace:: :menuselection:`&Insert-->Table of Contents and Inde&x-->&Bibliography Entry...`
.. |InsertBiblioEntryDlg| replace:: :guilabel:`Insert bibliography Entry` ダイアログボックス

書誌データベースから文献を文書に追加する手順：

#. キャレットを参照したい場所に置く。
#. |MenuBar| で |InsertBiblioEntryM| を選択。
...
#. |InsertTOCM| を選択し、|TypeList| を `Bibliography` に変更する。
...
#. |Sidebar| |StylesDeck| で |ListStylesTab| をクリックする。`Numbering 123` を
   右クリックし、|NewC| を選択する。
...
#. |OK| を押して設定を保存し、ダイアログボックスを閉じる。|StylesDeck| に戻り、
   ...
   し、`Bibliography 1` を右クリックして |EditStyleC| を選択する。
...
* |UpdateIndexC| で書誌を更新する。
* |EditIndexC| で |TOCDlg| を開き、書誌を編集・保存できる。
* 確認要求なしで書誌を削除するには |DeleteIndexC| を選択する。
