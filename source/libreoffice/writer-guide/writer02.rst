...
For&matting Marks` を選択する。|Chapter20| 参照。
...
選択モードを変更するには |StatusBar| の図像を使う方法がある。|Chapter01| で述べ
た欄を右クリックするとコンテキストメニューに選択肢が表示される：
...
#. 最初のテキストを選択
#. :kbd:`Shift` + :kbd:`F8` を押すか、|StatusBar| の :guilabel:`Selection mode`
...
される。また、ここにある :guilabel:`More Characters...` を押すとダイアログボック
ス :guilabel:`Special Characters` が開く。
...
.. tip::
   ...
   波下線に別の色を選択する方法は |Chapter20| で会得できる。

English sentence checking
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

追加的文法検査ツールを |OptionsDlg| の :menuselection:`Language
Settings-->English Sentence Checking` ページで選択可能だ。また、メニュー
...
可能だ（例えば、見出しに対してはしないようにする）。この話題は |Chapter08| と
|Chapter09| で述べられる。
...
|AutoCorrectDlg| については |Chapter04| でも述べられる。
...
AutoText はフィールドに割り当てると特に強力だ。|Chapter17| を読め。
