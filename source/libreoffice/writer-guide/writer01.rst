...
|StylesDeck|
   ...
   イル変更を行う。|Chapter08|, |Chapter09| で詳しく見る。
|GalleryDeck|
...
新規ツールバーを作成することが可能だ。|Chapter20| 参照。
...
単語数と文字数
   ...
   * ここに表示される文字数は空白を含む。空白を除いた文字数を表示するには、この
     区画をクリックするか、メニュー :menuselection:`&Tools-->&Word Count...` を
     実行しろ。
   * 文書全体の単語数と文字数（およびページ数、表組、画像、その他の統計）を表示
     するには |File-->Properties| を実行して |StatsTab| を見ろ。
Accessibility 検査状態
...
選択モード
   ...
   * :menuselection:`&Block selection`
   ...
   が表示され、クリックするとその選択モードになる。|Chapter03| 参照。図像はモー
...
* LibreOffice が起動しているが文書が開かれていない場合、Start Center が表示され
  ...
  る。
* メニュー :menuselection:`&File-->&New-->` を使い、サブメニューで文書の種類を選
  択する。
...
ついては |Chapter10| で見ていく。

|TemplateDlg| の開き方は次のいずれかだ：
...
.. note::
   ...
   宗主文書では Navigator 機能はここで述べられているものと異なる。|Chapter16| で
   見る。
...
.. note::
   ...
   :guilabel:`hidden` という文字が表示される。|Chapter06| 参照。
...
* |MenuBar| かタイトルバーの :kbd:`Close` 図像をクリック
