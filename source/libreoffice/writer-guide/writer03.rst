...
この章の読者が |Chapter02| に馴染んでいることを仮定される。

|OptionsDlg| |FormattingAidsPage| で段落末尾印、タブ、改行などの書式設定補助項目
を表示する設定にすることも推奨。

テキストの書式設定については |Chapter04| を見ろ。
...
サブ|MenuBar| の :menuselection:`&Reset to Default Language` オプションは、選択範
...
スタイルの言語設定を管理する方法の詳細については |Chapter09| で習う。
...
単語や語句の検索と置換 (|Chapter02|) に加えて、|Find&ReplaceDlg| を使って、段落
スタイルやテキスト書式の検索と置換を行うことが可能だ。
...
#. |TrackChangesToolbar| の :guilabel:`Manage Track Changes` 図像をクリックする
   か、メニューから :menuselection:`&Edit-->Track Chan&ges--> &Manage...` を選択
   する。
...
* ページ上の脚注の位置と、区切り線の色と線スタイルを定義する。|Chapter05| で述べ
  られる。
...
キストが更新されるようにするには自動相互参照を使用する。詳しくは |Chapter17| で
やる。
...
* |Chapter06| で説明する |InsertSectionDlg|
